...
def build_rows(items):
    ...
    for i, item in enumerate(items, 1):
        title = item.get("title") or item.get("raw", "")[:60] or "-"
        ...
        <tr>
            ...
            <td>
                <a href="{link}" target="_blank" class="btn-detail" rel="noopener">상세보기</a>
            </td>
        </tr>""")
...
<head>
  ...
  <style>
    * {{ box-sizing: border-box; margin: 0; padding: 0; }}
    ...
    header {{
      background: linear-gradient(135deg, #1a4fa0, #2d7dd2);
      color: white;
      padding: 28px 40px;
    }}
    ...
    @media (max-width: 768px) {{
      header, .stats, .toolbar, .container, .source-link {{
        padding-left: 16px;
        padding-right: 16px;
      }}
...
function filterTable() {{
  ...
  rows.forEach(row => {{
    ...
    const matchStatus = !status || badge.includes(status);
